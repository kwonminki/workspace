...
static void SetupEDMA3( void ){
    ...
    //For Receiver
    ConfigEDMAForR( R_LINK_PING, (Uint32)(ping.in_L), BUFLEN, ELEMENT_SIZE, R_LINK_PONG, TCINT );
    ConfigEDMAForR( R_LINK_PONG, (Uint32)(pong.in_L), BUFLEN, ELEMENT_SIZE, R_LINK_PING, TCINT );
                    //32번, 33번에 해당하는 RAMSET에 명령서를 작성하는 것이다. 서로에게 링크를 걸어둔다.
    ConfigEDMAForR( REVT, (Uint32)(ping.in_L), BUFLEN, ELEMENT_SIZE, R_LINK_PONG, TCINT );
                    //채널 0번 명령서인데 끝나면 퐁에게 링크된다. 즉 카피해와서 쓴다.
    //For Transmitter
    ConfigEDMAForX( X_LINK_PING, (Uint32)(ping.out_L), BUFLEN, ELEMENT_SIZE, X_LINK_PONG, TCINT );
    ConfigEDMAForX( X_LINK_PONG, (Uint32)(pong.out_L), BUFLEN, ELEMENT_SIZE, X_LINK_PING, TCINT );
                    //맞는 곳에 해당하는 RAMSET에 명령서를 작성하는 것이다. 서로에게 링크를 걸어둔다.
    ConfigEDMAForX( XEVT, (Uint32)(ping.out_L), BUFLEN, ELEMENT_SIZE, X_LINK_PONG, TCINT );
...
void main()
{
    ...
    puts("LAB7C - Ping-Pong Buffer Processing(EDMA, Linking, Channel Sorting, Structure)\n");
